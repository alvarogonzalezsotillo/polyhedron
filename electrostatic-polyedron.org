...
#+latex_header_extra: \lstset{frame=single,columns=fixed,basicstyle=\scriptsize\ttfamily,breaklines=true,postbreak=\raisebox{0ex}[0ex][0ex]{\ensuremath{\color{red}\hookrightarrow\space}}}
...
#+caption[Figuura]: Cálculo de la nueva posición de la carga $c_1$ para un total de 3 cargas
#+name: fig:algoritmo
...
Los ficheros =STL= generados pueden visualizarse con *OpenSCAD*, utilizando la orden =import=, como se muestra en el listado [[listadovisualizacionstl]]

la referencia rara [[(unareferenciainventada)]]

#+caption: Generación de los sólidos de ejemplo
#+name: listadovisualizacionstl
#+begin_src java
STLFILE="poliedro-10.stl";  (ref:unareferenciainventada)
ANGLE=20;
...
Las imágenes utilizadas en la tabla [[tablapoliedros]] se han generado con el programa del listado [[listadovisualizacionstl]] y el /script/ del listado [[lis:generacionimagenes]]

#+caption: Generación de los sólidos de ejemplo
#+name: lis:generacionimagenes
#+begin_src sh
#!/bin/bash -x
